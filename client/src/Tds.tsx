...
export function Tds() {
  ...
  const [graphTitle, setGraphTitle] = useState(make_time_title("TDS", "Hour"));
  ...
  useEffect(() => {
    fetch_slider_then_graph_data('tds', setMaxY, setMinY, setChartData, setReloadGraph, reloadGraph, setSliderValueWarn, setSliderValueAlert);
  ...
  const mock_data = async() => {
    try {
      const response = await fetch(`${backendAddress}/mock/tds`, {
        ...
        body: JSON.stringify([tds_min, tds_max, interval_min, time_frame]),
      });
    ...
    <>
      ...
      <div className='sensor-bkg'>
        ...
        <div className='btn-row'>
          <button id='hour-btn' className='time-btn time-pressed' onClick={() => {setReloadGraph(!reloadGraph); setTimeUnit('hour'); setTimeMin(new Date(new Date().getTime() - 1 * 60 * 60 * 1000).toISOString()); setGraphTitle(make_time_title("TDS", "Hour")); set_button_press_style('hour-btn')}}>Hour</button>
          <button id='day-btn' className='time-btn' onClick={() => {setReloadGraph(!reloadGraph); setTimeUnit('day'); setTimeMin(new Date(new Date().getTime() - 24 * 60 * 60 * 1000).toISOString()); setGraphTitle(make_time_title("TDS", "Day")); set_button_press_style('day-btn')}}>Day</button>
          <button id='week-btn' className='time-btn' onClick={() => {setReloadGraph(!reloadGraph); setTimeUnit('week'); setTimeMin(new Date(new Date().getTime() - 7 * 24 * 60 * 60 * 1000).toISOString()); setGraphTitle(make_time_title("TDS", "Week")); set_button_press_style('week-btn')}}>Week</button>
        </div>
      ...
      <div id="myModal" className="modal">
        <div className="modal-content">
          ...
          <button className='submit-param-btn' onClick={() => {upload_parameters('tds', 0, sliderValueWarn, sliderValueAlert); setReloadGraph(!reloadGraph); modal!.style.display = "none"}}>Submit</button>
        </div>
